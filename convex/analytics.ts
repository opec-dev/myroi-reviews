...
export const resellerOverview=query({args:{since:v.optional(v.number())},handler:async(ctx,args)=>{const user=await requireUser(ctx);if(!user.isPlatformAdmin)throw new Error("Administrator access required");const since=args.since??0;const [events,emails,settings]=await Promise.all([ctx.db.query("analyticsEvents").collect(),ctx.db.query("emailDeliveryLogs").collect(),ctx.db.query("resellerEmailSettings").withIndex("by_owner",q=>q.eq("ownerUserId",user._id)).unique()]);return{events:events.filter(x=>x.occurredAt>=since),emails:emails.filter(x=>x.occurredAt>=since),alertsAcknowledgedAt:settings?.alertsAcknowledgedAt??0}}});
