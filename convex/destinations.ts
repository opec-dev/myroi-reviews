...
export const listForBusiness = query({ args:{ businessId:v.id("businesses") }, handler:async(ctx,args)=>{ await requireBusinessAccess(ctx,args.businessId); const rows=await ctx.db.query("reviewDestinations").withIndex("by_business",q=>q.eq("businessId",args.businessId)).collect();return await Promise.all(rows.map(async(row)=>({...row,iconUrl:row.iconStorageId?await ctx.storage.getUrl(row.iconStorageId):null}))); }});
export const upsert = mutation({ args:{ id:v.optional(v.id("reviewDestinations")),businessId:v.id("businesses"),provider,label:v.string(),reviewUrl:v.string(),profileUrl:v.optional(v.string()),iconStorageId:v.optional(v.id("_storage")),displayOrder:v.number(),isEnabled:v.boolean() }, handler:async(ctx,args)=>{ await requireBusinessAccess(ctx,args.businessId); const {id,...values}=args; if(id){await ctx.db.patch(id,values);return id;} return await ctx.db.insert("reviewDestinations",values); }});
