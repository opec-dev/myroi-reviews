...
async function ensureYorkshireDiamondProvider(ctx:MutationCtx,businessId:Id<"businesses">){const rows=await ctx.db.query("reviewDestinations").withIndex("by_business",q=>q.eq("businessId",businessId)).collect();if(rows.some(row=>row.provider==="diamond"))return;await ctx.db.insert("reviewDestinations",{businessId,provider:"diamond",label:"Diamond Certified",reviewUrl:"https://www.diamondcertified.org/report/yorkshire-roofing-of-northern-california-inc-dba-roofmax/",profileUrl:"https://www.diamondcertified.org/report/yorkshire-roofing-of-northern-california-inc-dba-roofmax/",displayOrder:rows.length,isEnabled:false})}
...
export const bootstrapYorkshire = mutation({ args: {}, handler: async ctx => {
  ...
  if (existing){await Promise.all([seedYorkshireReviews(ctx,existing._id),ensureYorkshireDiamondProvider(ctx,existing._id)]);return existing._id}
  ...
  await Promise.all([
    ...
    ctx.db.insert("reviewDestinations", { businessId: created.businessId, provider: "diamond", label: "Diamond Certified", reviewUrl: "https://www.diamondcertified.org/report/yorkshire-roofing-of-northern-california-inc-dba-roofmax/", profileUrl: "https://www.diamondcertified.org/report/yorkshire-roofing-of-northern-california-inc-dba-roofmax/", displayOrder: 2, isEnabled: false }),
