...
export async function GET(){
  ...
  if(!settings?.smtpHost)missing.push("SMTP host");if(!settings?.smtpUser)missing.push("SMTP username");if(!settings?.smtpPasswordConfigured)missing.push("SMTP password");if(!settings?.fromEmail)missing.push("sender address");if(!settings?.smtpTestRecipient)missing.push("test recipient");
  return Response.json({configured:missing.length===0,missing,smtpHostConfigured:Boolean(settings?.smtpHost),smtpUserConfigured:Boolean(settings?.smtpUser),smtpPasswordConfigured:Boolean(settings?.smtpPasswordConfigured),smtpFromEmailConfigured:Boolean(settings?.fromEmail),smtpTestRecipientConfigured:Boolean(settings?.smtpTestRecipient),alertSmtpPasswordConfigured:Boolean(settings?.alertSmtpPasswordConfigured)},{headers:{"Cache-Control":"no-store"}});
}
