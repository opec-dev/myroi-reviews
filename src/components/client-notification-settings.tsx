...
export function ClientNotificationSettingsEditor({businessId,businessName}:{businessId:Id<"businesses">;businessName:string}){
  ...
  return <>
    ...
    <p className="panel-intro">Messages use the shared primary email provider and automatically retry through the backup provider when available.</p>
    ...
    <div className="notification-flow"><span><b>Delivery</b> Primary provider → backup provider if needed</span><strong>→</strong><span><b>Recipient</b> {settings.notificationEmail||"Client email not set"}</span></div>
