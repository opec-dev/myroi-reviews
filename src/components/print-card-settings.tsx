...
export function PrintCardSettingsEditor({businessId,slug}:{businessId:Id<"businesses">;slug:string}) {
  ...
  useEffect(() => {
    ...
    setSettings({...defaultPrintSettings,...savedSettings});
  }, [stored]);
  function change(event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) { const key = event.target.name as keyof PrintSettings; setSettings(current => ({ ...current, [key]: event.target.type === "range" ? Number(event.target.value) : event.target.value })); setSaved(false);setSaveMessage(""); }
  function toggleBadge(id:string) { setSettings(current => ({ ...current, platformBadges:current.platformBadges.includes(id) ? current.platformBadges.filter(x => x !== id) : [...current.platformBadges,id] })); setSaved(false);setSaveMessage(""); }
  async function save() { setSaving(true);setSaved(false);setSaveMessage("");try{await saveSettings({businessId,...settings});setSaved(true);setSaveMessage("Card settings saved.")}catch(error){setSaveMessage(error instanceof Error?error.message:"Card settings could not be saved.")}finally{setSaving(false)} }
  return <>
    ...
    <div className="settings-save"><small className={saveMessage&&!saved?"save-error":""}>{saveMessage||"Defaults are ready to customize."}</small><button className="button primary" disabled={saving} onClick={()=>void save()}>{saving?"Saving…":"Save card settings"}</button></div>
  </>;
}
