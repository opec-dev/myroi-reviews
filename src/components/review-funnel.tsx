...
export function ReviewFunnel({
  ...
  const connected = useQuery(api.funnel.publicBySlug, { slug });
  const saveFeedback = useMutation(api.funnel.submitPrivateFeedback);
  ...
  const [destinations, setDestinations] =
    useState<ReviewDestination[]>(demoDestinations);
  const [branding, setBranding] = useState(defaultBusinessBranding);
  const trackedView = useRef(false);
  const ratingSymbol = settings.ratingIcon === "heart" ? "♥" : "★";

  useEffect(() => {
    if (!connected) return;
    if (connected.funnel)
      setSettings({ ...defaultFunnelSettings, ...connected.funnel });
    setBranding({
      logoUrl: connected.business.logoUrl || defaultBusinessBranding.logoUrl,
      iconUrl: connected.business.iconUrl || defaultBusinessBranding.iconUrl,
      primaryColor: connected.business.primaryColor,
      secondaryColor:
        connected.business.secondaryColor || connected.business.primaryColor,
    });
    setDestinations(
      connected.destinations.map((row) => ({
        id: row._id,
    ...
    if (!trackedView.current) {
      ...
      trackPilotEvent({ businessSlug: slug, type: "funnel_view", source });
      if (
        source === "qr" &&
        new URLSearchParams(window.location.search).get("tracked") !== "1"
      )
        trackPilotEvent({ businessSlug: slug, type: "qr_scan", source });
    }
  }, [connected, slug]);
  ...
  async function submitPrivateFeedback(event: FormEvent<HTMLFormElement>) {
    ...
    const entry = {
      ...
      business: connected?.business.name ?? slug,
      slug,
    ...
    const recipient = entry.contact;
    const subject = `Private ${rating}-star feedback for ${entry.business}`;
    try {
      ...
      recordPilotEmail({
        ...
        status: result.notified ? "sent" : response.ok ? "not_sent" : "failed",
      ...
      recordPilotEmail({
        ...
        recipient,
        subject,
        error: "Delivery endpoint unavailable",
      });
    }
    setStage("complete");
  }
  ...
  return (
    <main
      ...
      style={{ "--primary": branding.primaryColor } as CSSProperties}
    >
      {preview && (
        <a
          className="preview-back public-preview-back"
          href={`/client/${slug}#overview`}
        >
      ...
      <section className="review-funnel">
        <div className="logo-lockup">
          <img
            ...
            alt={`${connected?.business.name ?? slug} logo`}
          />
        </div>
        {settings.showBusinessName && (
          <strong className="funnel-business-name">
            {connected?.business.name ?? slug}
          </strong>
        )}
          ...
          <div className="funnel-stage">
            ...
            <div className="rating-picker" onMouseLeave={() => setHovered(0)}>
              {[1, 2, 3, 4, 5].map((value) => (
                <button
                  key={value}
                  aria-label={`${value} ${settings.ratingIcon}${value === 1 ? "" : "s"}`}
                  onMouseEnter={() => setHovered(value)}
                  onClick={() => chooseRating(value)}
                  className={value <= (hovered || rating) ? "selected" : ""}
                >
                  {ratingSymbol}
                </button>
              ))}
        ...
        {stage === "positive" && (
          <div className="funnel-stage">
            ...
            <h1>{settings.positiveHeadline}</h1>
            ...
            >
              {settings.maybeLaterText}
            </button>
          </div>
        )}

        {stage === "recovery" && (
          <div className="funnel-stage recovery-stage">
            <h1>{settings.recoveryHeadline}</h1>
            ...
            <button
              className="quiet-button public-review-link"
              onClick={() => {
                trackPilotEvent({
                  businessSlug: slug,
                  type: "public_review_fallback",
                  rating,
                });
                setStage("positive");
              }}
            >
              {settings.publicLinkText}
            </button>
          </div>
        )}

        {stage === "complete" && (
          <div className="funnel-stage completion-stage">
            <div className="success-mark">✓</div>
            <h1>{settings.completionHeadline}</h1>
            <p>{settings.completionSubtext}</p>
          </div>
        )}
        {!!connected?.business.socialLinks?.length && (
          <div className="funnel-socials">
            <small>Follow us</small>
            <div>
              {connected.business.socialLinks.map((link) => (
                <a
