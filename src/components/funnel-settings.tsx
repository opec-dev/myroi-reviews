...
export function FunnelSettingsEditor({businessId,business}:{businessId:Id<"businesses">;business:{name:string;slug:string;logoUrl?:string|null}}) {
  const stored=useQuery(api.funnel.forBusiness,{businessId}); const saveSettings=useMutation(api.funnel.saveSettings);
...
function FunnelMiniPreview({ settings, stage,business }: { settings:FunnelSettings; stage:PreviewStage;business:{name:string;logoUrl?:string|null} }) {
  return <aside className="live-preview-sticky"><span className="eyebrow">Live funnel preview</span><div className="mini-funnel"><img src={business.logoUrl||"/brands/yorkshire-roofing.png"} alt="" />{settings.showBusinessName && <strong>{business.name}</strong>}{stage === "rating" && <><h3>{settings.ratingHeadline}</h3><p>{settings.ratingSubtext}</p><div className="mini-stars">★★★★★</div><small>Tap a star to rate your experience</small></>}{stage === "positive" && <><span className="preview-mark">♥</span><h3>{settings.positiveHeadline}</h3><p>{settings.positiveSubtext}</p><button>Review us on Google</button><button>Review us on Yelp</button><small>{settings.maybeLaterText}</small></>}{stage === "recovery" && <><h3>{settings.recoveryHeadline}</h3><p>{settings.recoverySubtext}</p><input readOnly placeholder={settings.nameLabel} /><input readOnly placeholder={settings.contactLabel} /><textarea readOnly placeholder={settings.messageLabel} /><button>{settings.submitText}</button><small>{settings.publicLinkText}</small></>}{stage === "complete" && <><span className="preview-mark">✓</span><h3>{settings.completionHeadline}</h3><p>{settings.completionSubtext}</p></>}</div></aside>;
}
